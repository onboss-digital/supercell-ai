...
function CRM() {
  ...
  return (
    <main className="main-content" style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', padding: 0, position: 'relative' }}>
                ...
                <tbody>
                  {filteredLeads.map(lead => (
                    <tr key={lead.id} onClick={() => setSelectedLead(lead)} style={{ background: selectedLead?.id === lead.id ? 'var(--color-surface-container-high)' : 'var(--color-surface-container-lowest)', cursor: 'pointer', transition: '0.2s' }}>
                      ...
                      <td style={{ padding: '0.75rem 0.5rem', fontWeight: '700', fontSize: '0.75rem', color: 'var(--color-primary)' }}>{lead.phone}</td>
      ...
      {/* Selected Lead Modal */}
      {selectedLead && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1200, padding: '1.5rem' }} onClick={() => setSelectedLead(null)}>
            <div style={{ width: '100%', maxWidth: '600px', background: 'white', borderRadius: '2rem', display: 'flex', flexDirection: 'column', padding: '2.5rem', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', position: 'relative', animation: 'modalFadeIn 0.3s ease-out' }} onClick={(e) => e.stopPropagation()}>
               ...
               <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ background: selectedLead.platform === 'instagram' ? 'linear-gradient(45deg, #f09433 0%, #bc1888 100%)' : '#25D366', color: 'white', padding: '0.4rem', borderRadius: '50%', display: 'flex' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', fontWeight: '900', color: '#64748b', letterSpacing: '0.1em' }}>REVISÃO DE LEAD - {selectedLead.platform.toUpperCase()}</span>
                  </div>
                  <button onClick={() => setSelectedLead(null)} style={{ background: '#f1f5f9', border: 'none', cursor: 'pointer', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                     <span className="material-icons-outlined" style={{ fontSize: '1.2rem', color: '#64748b' }}>close</span>
                  </button>
               </div>
               <div style={{ marginBottom: '2.5rem' }}>
                  <h3 style={{ fontSize: '2.25rem', fontWeight: '900', marginBottom: '0.5rem', color: '#0f172a' }}>{selectedLead.name}</h3>
                  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                     <span style={{ fontSize: '1.1rem', fontWeight: '700', color: 'var(--color-primary)' }}>{selectedLead.phone}</span>
                     <span style={{ width: '4px', height: '4px', borderRadius: '50%', background: '#cbd5e1' }}></span>
                     <span style={{ fontSize: '0.9rem', color: '#0ea5e9', fontWeight: '800' }}>{selectedLead.campaignName || 'Orgânico'}</span>
                  </div>
               </div>
               <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                  <div style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)', padding: '2rem', borderRadius: '1.5rem', border: '1px solid #e2e8f0' }}>
                     <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', color: 'var(--color-primary)', marginBottom: '1rem' }}>
                        <span className="material-icons-outlined" style={{ fontSize: '1.4rem' }}>auto_awesome</span>
                        <p style={{ fontSize: '0.9rem', fontWeight: '900' }}>MENSAGEM DO JARVIS</p>
                     </div>
                     <p style={{ fontSize: '1.1rem', lineHeight: '1.7', color: '#334155' }}>
                        Lead identificado via <strong>{selectedLead.platform}</strong>. Campanha: <strong>{selectedLead.campaignName || 'Direta'}</strong>.
                        <br />Tática recomendada: Inicie o chat oferecendo as promoções vigentes para o canal.
                     </p>
                  </div>
                  <div style={{ display: 'flex', gap: '1rem' }}>
                     <a href={selectedLead.platform === 'instagram' ? `https://instagram.com/direct/t/${selectedLead.phone}` : `https://wa.me/${selectedLead.phone ? selectedLead.phone.replace(/\D/g, '') : ''}`} target="_blank" rel="noreferrer" style={{ flex: 2, textDecoration: 'none', padding: '1.25rem', borderRadius: '1rem', background: selectedLead.platform === 'instagram' ? 'linear-gradient(45deg, #f09433 0%, #dc2743 100%)' : '#25D366', color: 'white', fontWeight: '900', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem' }}>
                        {selectedLead.platform === 'instagram' ? 'ABRIR DIRECT' : 'ABRIR WHATSAPP'}
                     </a>
                     <button onClick={() => setSelectedLead(null)} style={{ flex: 1, padding: '1.25rem', borderRadius: '1rem', border: '1px solid #e2e8f0', background: 'white', color: '#64748b', fontWeight: '900' }}>FECHAR</button>
                  </div>
               </div>
            </div>
          </div>
